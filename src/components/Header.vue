  ...
  <div class="container">
    <div class="flex items-center justify-between border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
      ...
      <nav class="hidden space-x-10 md:flex">
        <RouterLink :to="{ name: 'categories' }" class="text-base font-medium text-gray-500 hover:text-gray-900">Categories</RouterLink>
      </nav>

      <div class="hidden items-center justify-end md:flex md:flex-1 lg:w-0">
        <template v-if="loggedIn">
          <RouterLink :to="{ name: 'post-create' }" class="inline-flex items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-700">Create New Post</RouterLink>
        </template>
        <template v-else>
          <RouterLink to="/signin" class="whitespace-nowrap text-base font-medium text-gray-500 hover:text-gray-900">Sign in</RouterLink>
          <RouterLink to="/signup" class="ml-8 inline-flex items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-700">Sign up</RouterLink>
        </template>
      </div>
    </div>
  </div>
